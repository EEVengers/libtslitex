...
void litepcie_dma_cleanup(struct litepcie_dma_ctrl *dma)
{
    ...
    if (dma->use_writer)
        litepcie_dma_writer(dma, 0, &dma->writer_hw_count, &dma->writer_sw_count, &dma->writer_dropped_count);
...
void litepcie_dma_process(struct litepcie_dma_ctrl *dma)
{
    ...
    /* set / get dma */
    if (dma->use_writer)
        litepcie_dma_writer(dma, 1, &dma->writer_hw_count, &dma->writer_sw_count, &dma->writer_dropped_count);
    if (dma->use_reader)
